...
for cols in outlier_columns:
    for label in df["label"].unique():
        dataset = mark_outliers_chauvenet(df[df["label"] == label], cols)
        dataset.loc[dataset[cols + "_outlier"], cols] = np.nan
        outliers_removed_df.loc[outliers_removed_df["label"] == label, cols] = dataset[
            col
        ]
        n_outliers = len(dataset) - len(dataset[cols].dropna())
